...
void CommunicatorXP::cutRTPheader(unsigned char* data,unsigned length) {
    ...
    int i=0;
    uint8_t sliceBegins = 0;
    uint8_t sliceEnds = 0;
    uint8_t mpegInfoByte;
    
    //rtpheader = (unsigned char*)malloc(16);
    //newframe = (unsigned char*)malloc(1500);
    uint8_t rtpheader[16];
    uint8_t newframe[1500];
    //printf(". ");
    //printf("New data length is: %u\n",length);

    //newframe = HEADER_SEQUENCE;
    memcpy(newframe, HEADER_SEQUENCE, 5);
    if(length>=20) {
    memcpy(rtpheader,data,16); // copies first 16 bytes (rtpheader)
    memcpy(newframe+5,rtpheader,16);
    
    mpegInfoByte = data[14];
    sliceBegins = (mpegInfoByte>>4) & 0x01;
    sliceEnds   = (mpegInfoByte>>3) & 0x01;
    
    //printf("Infobyte: %02X - Slice Begins: %02X - Slice Ends: %02X\n", mpegInfoByte, sliceBegins, sliceEnds);

    if (data[1] == 0x20 && length>16) {  //first check .. is video
      if((sliceBegins && sliceEnds) || (sliceBegins && !sliceEnds)) {
        //printf("Slice Begins and Ends or Begins and doesn't end ...\n");
      ...
      else if(!sliceBegins && sliceEnds) {
        //printf("Slice doesn't begin but ends here ...\n");
        mutexBufferV->write(data+16,length-16, 0); // writes received packet into buffer...
      }
      else if(!sliceBegins && !sliceEnds) {
      /* we do not know what to do here .. packet is broken (we think so ...) */
        //printf("FAILURE FAILURE FAILURE but packet length was %i\n\n",length);
	mutexBufferV->write(newframe, 21, 0); // writes emulated "B4" packet into buffer...
        mutexBufferV->write(data+16,length-15, 0); // writes received packet into buffer...
      }
